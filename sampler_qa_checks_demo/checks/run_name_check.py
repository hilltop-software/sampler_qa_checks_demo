...
class RunNameCheck(ICheck):
    ...
    def __init__(self, config, repository):
        ...
        if self.disabled:
            return
        self.name_max_length = self.config.get(
            "name_max_length", 100
        )  # default to 100 characters
        HilltopHost.LogInfo(
            f"sampler_qa_checks_demo - RunNameCheck is using a limit of {self.name_max_length} characters"
        )
    ...
    def perform_checks(self, run_id, context) -> List[QACheck]:
        ...
        if len(context.RunName) > self.name_max_length:
            ...
            qa_check.Details = (
                f"Run name '{context.RunName}' is {len(context.RunName)} characters long,"
                f"which exceeds the maximum of {self.name_max_length}."
            )
